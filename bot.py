...
# =========================
ADMIN_ID = 8646015281
BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "8796716322:AAGqHq-kXrehQ7YT3i5372sXvCgs6kG87X4")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(update, Update):
        try:
            if update.callback_query:
                await update.callback_query.answer("Ocorreu um erro. Tente novamente.", show_alert=True)
                await update.callback_query.message.reply_text(
                    "⚠️ Não foi possível concluir esta ação. Verifique o Firebase e tente novamente."
                )
            elif update.effective_message:
